...
#be sure to have correct packages installed!
...
temperatures <- read_csv("https://wilkelab.org/SDS375/datasets/tempnormals.csv") %>%
  ...
  ) %>%       #this is the old piping symbol, can be used along w/ new version (|>) 
...
```{r ggplot-hint}
p <- ggplot(temps_houston, aes(x = day_of_year, y = temperature)) +
  geom_line()
p <- p + ylab("TEST") #changing y axis label 
p
```
...
ggplot(temps_houston, aes(x = day_of_year, y = temperature)) +
  geom_line()
#note that variables assigned to x & y are NOT in quotations
#we are not assigning a string or title, rather assigning x & y to variables extracted from main data set
#recall that main data set is read from the csv file in the first chunk
#this way, we can choose 2 subsets to visualize 
```
...
ggplot(temps_houston, aes(x = day_of_year, y = temperature)) +
  geom_point()
```
Thicker line!! - specific points making up line trend
...
```{r ggplot3-solution}
p <- ggplot(temps_houston, aes(x = temperature, y = day_of_year)) +
  geom_point()
```
...
ggplot(temps_houston, aes(x = month, y = temperature,col = 'red')) +
  geom_boxplot()
```
...
ggplot(temperatures, aes(x = day_of_year, y = temperature, color = ___)) +
  geom_line()
```
Adding a name to the plot...
```{r ggplot-color-solution}
p <- ggplot(temperatures, aes(x = day_of_year, y = temperature, color = location)) +
  geom_line()
p
```
