...
p <- ggplot(lincoln_temps, aes(x = month, y = mean_temp)) +
...
ggplot(lincoln_temps, aes(x = mean_temp, y = month_long)) +
  geom_density_ridges(rel_min_height = 0.01, fill="dodgerblue2")
```

Now, let's suppose we want to save the figure to use within a google slideshow or a lab report. You can save the file in multiple formats. Here, let's just look at saving as a pdf (*if you wanted to save as a png, just change file extention in name). First, give the plot a variable name. Next, call ggsave. You can explore in more detail how to control the size of the saved image and other characteristics in Wilke's book. 
```{r lincoln-ridges3-save-file}
pridge <- ggplot(lincoln_temps, aes(x = mean_temp, y = month_long)) +
  geom_density_ridges(rel_min_height = 0.01, fill="dodgerblue2")

ggsave("myplot2.png", plot = pridge)
```